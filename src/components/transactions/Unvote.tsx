...
export const Unvote = (props: {username : string, setShow: (b:boolean) => void}) => {
    ...
    function validateSend(fees:string) {
      ...
      function unvote() {
        Armor.currentWallet().then((wallet) => {
            wallet.unvote(delegateInfo.publicKey,fee, password).then(() => {
                props.setShow(false);
            })
        })
      }
   ...
   if (delegateInfo && currentWallet) {return (
      ...
      />
        ...
        <div className="py-2">
        <Button className="w-fit text-center mx-auto" onClick={unvote} disabled={!sendEnabled} > Unvote {props.username} </Button>
        </div>
  </div>
    ); } else {
        return ( <div></div> )
